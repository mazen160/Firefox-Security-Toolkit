...
scriptpath=("/tmp/firefoxsecuritytoolkit/")


echo -en "[#]Click [Enter] to start. "; read
echo -e "[*]Creating a tmp directory at [$scriptpath]."
##Creating directory.
mkdir -p $scriptpath &> /dev/null
...
echo -n "[@]Would you like to download Burp Suite Certificate? [y/n]. Note that Burp Suite should be running in your machine. "; read burp_cert_answer
...
echo -n "[@]Would you like to download user-agent list for User-Agent Switcher Addon? [y/n]"; read useragent_list_answer
...
echo -en "[**]Click [Enter] to run Firefox to finish the task. "; read
